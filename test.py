...
class Test:
    def __init__(self, id: int, questions: List[Question], title: str):
        ...
        self.id = id
        # create a list with size answers
        self.correct_answers = list(range(len(questions)))

    def check_test(self):
        for question in self.questions:
            self.correct_answers[self.questions.index(
                question)] = "Correct" if question.is_answer_correct() else "Incorrect"
            # set whether current question is correct

    # answer amount should be identical to question amount
    ...
    def delete(self):
        with DB() as database:
            ...
            for question in self.questions:
                database.execute(
                    '''DELETE FROM test_questions WHERE test_id = ?''', (self.id,))
    ...
    def get_all():
        with DB() as database:
            rows = database.execute('''SELECT * FROM tests''').fetchall(
            )  # fetches all rows which correspond to the query (i.e. every row)
    ...
    def find(id):
        with DB() as database:
            row = Adapter.adapt_query(database.execute(
                '''SELECT * FROM tests WHERE id = ?''', (id,)).fetchall())
            ...
            question_order = Adapter.adapt_list_by_step_3(
                question_order)  # create list of lists spliced by 3

            print(question_order)

            try:
                row = tuple(Adapter.adapt_test_rows(
                    database, row, Question.get_test_questions(question_order[id - 1]))[0])
            except IndexError:
                row = tuple(Adapter.adapt_test_rows(
                   database, row, Question.get_test_questions(question_order[0]))[0])

            # fetch a single row where id is given id
            # sql queries take tuples, which is why we pass arguments in such a manner

            # return new instance of Test() where we pass the elements of the container as args
            return Test(*row)
