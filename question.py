...
class Question:
    ...
    def __init__(self, id: int, question: str, answers: List[str], correct_answer_index: int):
        ...
        self.correct_answer_index = 0 \
            if int(correct_answer_index) <= 0 or int(correct_answer_index) > 3 \
            else int(correct_answer_index) - 1

        print("kur answer %s" % str(self.correct_answer_index))
    ...
    def create(self):  # will probably bug out because there is no insertion or reference to the other table apart from the for loop
        with DB() as database:
            for ans in self.answers:
                database.execute(
                    '''INSERT INTO answers (id, answer) VALUES (?, ?)''', (self.id, ans))
    ...
    def delete(self):
        with DB() as database:
            database.execute(
                '''DELETE FROM questions WHERE id = ?''', (self.id,))

    @staticmethod
    def get_all():
        with DB() as database:
            rows = Adapter.adapt_query(database.execute(
                '''SELECT * FROM questions''').fetchall())  # convert the data into a list of lists
            rows = Adapter.adapt_question_rows(database, rows)

            # instantiate questions list
            return [Question(*row) for row in rows]

    @staticmethod
    # takes question ids and returns a list of questions for the given test
    def get_test_questions(ids):
    ...
    def find(id):
        with DB() as database:
            row = Adapter.adapt_query(database.execute(
                '''SELECT * FROM questions WHERE id = ?''', (id,)).fetchall())
